...
export const getObjectSchema = (schema, key, name, custom, options) => {
  ...
  if (typeDef(schema._schema, getObjectSchemaKey).constructor.name === 'SimpleSchema') {
    ...
    content = objectKeys[key + splitter].map(
      k => `${getFieldSchema(objectSchema, k, name, custom)}`,
    );
  } else {
    content = objectKeys[key + splitter].map(
      k => `${getFieldSchema(schema, `${key + splitter + k}`, name, custom)}`,
    );
  }
  ...
  return `
  ${options.inputType ? 'input' : 'type'} ${subGqlType(key, name)} {
    ${content}
  }
  `;
};
